...
The notation convention used is:
 ...
 `p =` number of variables, ie dimension of the data
...
1. Generate a matrix `M` with 5 rows and 2 columns, where each value is randomly drawn from a standard normal distribution. Extract the element at row 3 and column 1.
2. We will interpret `M` as a projection matrix and therefore it needs to be orthonormalised. Use the function `tourr::orthonormalise` to do this, and explicitly check that each column is normalised and that the two columns are orthogonal now. Which dimensions contribute most to the projection for your `M`?
3. Use matrix multiplication to calculate the projection of the `mulgar::clusters` data onto the 2D plane defined by `M`. Make a scatterplot of the projected data. Can you identify clustering in this view?
