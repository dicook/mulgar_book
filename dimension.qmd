...
#| eval: false
#| code-fold: false
animate_xy(aflw_std[,7:35], half_range=0.8)
```
...
The data is surprisingly good! There is a small amount of skewness, and there are no major outliers. Skewness indicates that most players have reasonably similar skills (bunching of points), except for a few key players (outliers. The skewness could be reduced by applying a log or square root transformation to some variables prior to running the PCA, but there are no major issues that would affect PCA.
...
aflw_pctour <- ggtour(aflw_pct, aflw_pca$x[,1:4], 
  ...
  proto_default()
aflw_pctourgg <- animate_plotly(aflw_pctour,
                                width=700, height=450)
htmlwidgets::saveWidget(aflw_pctourgg,
          file="html/aflw_pca.html",
          selfcontained = TRUE)
```

```{=html}
<iframe width="750" height="500" src="html/aflw_pca.html" title="Animation of AFLW four PCs with interactive labelling. "></iframe>
```
