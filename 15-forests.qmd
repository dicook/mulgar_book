...
\index{software!rpart}
\index{software!rpart.plot}
\index{software!classifly}
\index{software!ggdendro}
...
![Tree model](gifs/penguins_tree_boundaries.gif){#fig-tree-boundary fig-alt="Tour animation showing many 2D projections of points in a 4D cube. Points are divided into three regions, in directions parallel to different coordinate axes, giving boxier regions than the LDA boundaries." width=300}
...
![LDA model](images/penguins_lda_boundaries.png){#fig-lda-boundary fig-alt="This image shows a scatter plot of penguin data of three distinct penguin species. The points are divided into three regions like a pie, with red (Gentoo) at the top, blue (Adelie) on bottom right, and yellow (Chinstrap) on the left."}

![Tree model](images/penguins_tree_boundaries.png){#fig-tree-boundary fig-alt="This image shows a scatter plot of penguin data of three distinct penguin species. The points are divided into three boxes, with red (Gentoo) being a rectangule box at bottom left, blue (Adelie) a smaller square at top left, and yellow (Chinstrap) a smaller square at top right."}

Comparison of the boundaries produced by the LDA (a) and the tree (b) model, using a slice tour. (Here only a single frame is shown.) The tree boundaries are more box-shaped than the LDA boundaries, which does not adequately capture the differences between the elliptically-shaped clusters of the penguins data. {{< fa play-circle >}}
:::
:::
...
![3D](gifs/penguins_rf_votes.gif){#fig-p-votes-tour fig-alt="Tour animation showing many 2D projections of points from 3D. Points are fall mostly on the edges of a 2D triangle in 3D. Red (Gentoo) is a tight cluster at one vertex. Blue (Adelie) and yellow (Chinstrap) concentrate at the other two vertices, with points intermixed spread all along the edge between the two vertices. There is one red point mixed with the yellow, and one blue point along the yellow-red edge, and one blue point in the middle of the triangle." width=300}
...
![3D](images/penguins_rf_votes.png){#fig-p-votes-tour fig-alt="One 2D projection from a tour of the votes matrix, showing all points lie in a line. Red (Gentoo) is at the left end, blue (Adelie) is at the right end and yellow (Chinstrap) is on the middle. All three axes are pointing downwards: Gentoo to 8 o'clock, Chinstrap to 6 o'clock, Adelie to 5 o'clock."}

![2D ternary diagram](images/fig-p-votes-ggplot-1.png){#fig-p-votes-ggplot-pdf fig-alt="Black equilateral triangle surrounding points. Points are fall mostly on the edges of a 2D triangle in 3D. Red (Gentoo) is a tight cluster at one vertex. Blue (Adelie) and yellow (Chinstrap) concentrate at the other two vertices, with points intermixed spread all along the edge between the two vertices. There is one red point mixed with the yellow, and one blue point along the yellow-red edge, and one blue point in the middle of the triangle."}

Examining the votes matrix from a random forest fit to the penguins: (a) in a frame from a tour of the 3D, (b) projected into 2D, to make a ternary diagram. In 3D the points can be seen to lie along a 2D plane, which is due to the constraint that the values sum to 1. From the ternary diagram, the classification can be seen to be reasonably well distinguished because points mostly lie at the vertex. There are a few penguins that are confused with a different species, as seen from the few points spread between vertices. {{< fa play-circle >}}
:::
:::
...
![The 9D votes matrix for the 10 class fake_trees data in a tour.](gifs/ft_votes.gif){#fig-ft-votes-tour fig-alt="Animation of 2D projections of 10D data falling inside a 10D simplex. Vertices are labelled 0-9, corresponding also to classes 0-9, and points are coloured different shades of yellow-green-purple. Edges of the simplex are dark purple. Points fall mostly along several edges. Particularly, points tend to lie on a edge connecting a vertices 1 through 9 with the 0 vertex." width=300}

![Several static views from the tour revealing how clusters connect.](images/ft-votes.png){#fig-ft-votes-prj fig-alt="Four plots showing 2D projections of a 10D simplex. Vertices are labelled 0-9, corresponding also to classes 0-9, and points are coloured different shades of yellow-green-purple. Edges of the simplex are dark purple. Points fall mostly along several edges. Particularly, points tend to lie on a edge connecting a vertices 1 through 9 with the 0 vertex." width=300}
...
![](images/ft-votes.png){fig-alt="Four plots showing 2D projections of a 10D simplex. Vertices are labelled 0-9, corresponding also to classes 0-9, and points are coloured different shades of yellow-green-purple. Edges of the simplex are dark purple. Points fall mostly along several edges. Particularly, points tend to lie on a edge connecting a vertices 1 through 9 with the 0 vertex." width=400}

Several static views from the tour of the votes matrix. Lines are the edges of the 8D simplex, which bounds the shape. Points mostly concentrate in the vertices, or spread along one of the edges, which means that most observations are clearly belonging to one group, or confused with a single other group. The exception to this is class 0, which spreads in many directions. {{< fa play-circle >}}
:::
:::
...
Points concentrate at the vertices, which means that most are confidently predicted to be their true class. The most spread of points is along single edges, between pairs of vertices. This means that when there is confusion it is mostly with just one other group. One vertex (0) has connections to all other vertices. That is, there are points stretching from this vertex to every other. It means that some observations in every other class can be confused with class 0, and class 0 observations can be confused with every other class. This information suggests that cluster 0 is central to all the other clusters. 
...
![Tour of most important variables for class 1.](gifs/ft_cl1.gif){#fig-ft-cl1 fig-alt="Animation of 2D projections of 6D data with thickish branching structure. There are 9 branches attached to a main trunk. Variables are principal components. Most of the points are yellow, one branch is green, and the main trunk is deep purple." width=300}
...
![](images/ft_cl1.png){fig-alt="This is scatterplot of a projection from 6D. PC1 is pointing to 5 o'clock, PC2 is pointing to 8 o'clock, PC4 is pointing to 2 o'clock, and PC6 is pointing to 3 o'clock. Points are coloured by class groupings, deep purple is class 0, the main trunk, green is class 1, a branch, and all other classes are yellow. The class 1 is distinct from the other points pointing to 2 o'clock, and connected to one end of cluster 0."}


![](images/fig-ft-cl1-pc-1.png){fig-alt="This is an untitled chart with no subtitle or caption. It has x-axis 'PC4' with labels -5, 0 and 5. It has y-axis 'PC1' with labels -5, 0 and 5. Points are coloured by class groupings, deep purple is class 0, the main trunk, green is class 1, a branch, and all other classes are yellow. The class 1 is distinct from the other points pointing to 5 o'clock, and connected to one end of cluster 0."} 

Focusing on class 1 in the `fake_trees` data. The most important variables were PC1 and PC4. A combination of PC2 and PC4 reveals the difference between cluster 1 and all the other clusters. {{< fa play-circle >}}
:::
:::

For a problem like this, it can be useful to look at several classes together. We've chosen to start with class 8 (light green), because from `r ifelse(knitr::is_html_output(), '@fig-ft-votes-html', '@fig-ft-votes-pdf')` it appears to have less connection with class 0, and closer connection with another class. This is class 6 (medium green) which also has one observation confused with class 8 according to the confusion matrix (printed below). 
...
![Tour of most important variables for class 1.](gifs/ft_cl8.gif){#fig-ft-cl8 fig-alt="Animation of 2D projections of 6D data with thickish branching structure. There are 9 branches attached to a main trunk. Variables are principal components. Most of the points are yellow, the main trunk (class 0) is deep purple, and three branches that all connect at one end are different shaes of green (classes 1, 6, 8)." width=300}
...
![](images/ft_cl8.png){fig-alt="This is scatterplot of a projection from 6D. PC1 and PC2 are pointing to 4 o'clock, PC5 is pointing to 7 o'clock, and PC6 is pointing to 1 o'clock. Points are coloured by class groupings, deep purple is class 0, the main trunk, the rest are branches, where light green is class 8, middle green is class 6 and dark green is class 1. All other classes are yellow. The classes 1, 6, 8 are each distinct from the other points and connected to one end of cluster 0."}

![](images/fig-ft-cl8-pc-1.png){fig-alt="This is an untitled chart with no subtitle or caption. It has x-axis 'PC1' with labels -5, 0 and 5. It has y-axis 'PC6' with labels -6, -3, 0 and 3, 6. Points are coloured by class groupings, deep purple is class 0, the main trunk, the rest are branches, where light green is class 8, middle green is class 6 and dark green is class 1. All other classes are yellow. The classes 1, 6, 8 are each distinct from the other points and connected to one end of cluster 0."}

Focusing on class 8 in the `fake_trees` data using a tour (left) reveals that it shares an end of cluster 0 with clusters 1 and 6. A combination of PC1 and PC5 reveals that there is a difference between the observations in class 8 relative to 6, 1 and 0 is largely due to PC5 (right). {{< fa play-circle >}}
:::
:::
...
6. Using the `aflw` data (using the same subset of variables and players as used in Q3 of @sec-class-intro) compute:

    - a 2D representation using UMAP. (Make sure `position` is removed for the calculation.) 
    - Fit a random forest model.

    a. Plot the UMAP layout with points coloured by `position`.
    b. Plot the *two* most important variables as measured by the random forest model.
    c. Make a tour using the most important variables as measured by the random forest model, the top three should be enough.

    Discuss what is learned from each representation of the data, relative to the differences between skills of the players.
...
7. There are several interesting data sets with class variables available on the [GGobi website](http://ggobi.org/book/index.html). Examine the differences between `type` of music, based on the the variables `lvar`, `lave`, `lmax`, `lfener`, `lfreq`. Fit a random forest model using `type` as the variable. (It is best to remove the "New wave" class because there are too few observations.) Using the variable importance values, make plots of the top two variables, and then tour on the top three variables. Is two or three variables suffficient to distinguish between classical and rock music types?
