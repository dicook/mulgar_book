...
The boundaries for the tree model on all four variables of the penguins data can be viewed similarly, by predicting a set of points randomly generated in the 4D domain of observed values. `r ifelse(knitr::is_html_output(), '@fig-penguins-lda-tree-html', '@fig-penguins-lda-tree-pdf')` shows the prediction regions for LDA and a default tree in a slice tour. The default fitted tree is delightfully simple - it has just six splits of the data. However, the result is not adequate for the penguins data, because it is generating boxy boundaries between elliptically shaped clusters. 
