...
7. There are several interesting data sets with class variables available on the [GGobi website](http://ggobi.org/book/index.html). Examine the differences between `type` of music, based on the variables `lvar`, `lave`, `lmax`, `lfener`, `lfreq`. Fit a random forest model using `type` as the variable. (It is best to remove the "New wave" class because there are too few observations.) Using the variable importance values, make plots of the top two variables, and then tour on the top three variables. Is two or three variables sufficient to distinguish between classical and rock music types?
