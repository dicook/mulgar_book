...
We'll start by showing PCA on the simulated data from @sec-dimension-overview. The scree plots show that PCA supports that the data are 2D, 3D and 5D respectively.
...
#| tbl-cap: "Coefficients for the first four PCs for the plane data with two noise variables."
#| code-summary: "Code to print PC coefficients"
pn_pca$rotation[,1:4] %>%
  as_tibble(rownames="Variable") %>% 
  gt() %>%
...
Scree plot and a frame from a tour of the `pisa` data, with 30 variables being the plausible scores for Australian students. In combination, these suggest that the data is effectively 1D. {{< fa caret-right >}}
:::
:::
...
#| message: false
#| warning: false
#| fig-cap: "Frame 18 re-plotted so that players can be identified, ideally using on mouse-over. Here some points are labelled."
#| fig-width: 5
#| fig-height: 4
#| out-width: 80%
...
PCA model overlaid on the data for the 2D in 5D, and 3D in 5D simulated data. {{< fa caret-right >}}
:::
:::
...
PCA model of the `pisa` data. The 1D model captures the primary variation in the data and there is a small amount of spread in all directions away from the model. {{< fa caret-right >}}
:::
:::
...
PCA model of the `aflw` data. The linear model is not ideal for this data, which has other patterns like outliers, and some branching. However, the model roughly captures the linear associations, and leaves unexplained variation in different directions. {{< fa caret-right >}}
:::
:::
...
Examining the handling of outliers in the PCA of the planar data with noise variables and two outliers. PCA has lost these two extreme values. {{< fa caret-right >}}
:::
:::
...
Comparison of the full data and first three principal components. Non-linear relationships between several variables can be seen in a tour on all five variables. The first three principal components reveal a strong non-linear relationship. Some of the non-linearity is clearly visible in the reduced dimension space, but the full data has more complexities. {{< fa caret-right >}}
:::
:::
