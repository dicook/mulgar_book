...
![Map in 4D](gifs/p_som.gif){#fig-p-som1 fig-alt="Animation showing 2D projections from 4D of the penguins data, with points coloured according to species. Overlaid is a wireframe net, which stretches within and across the clusters. A circle and segment display shows the coefficients for each projection at bottom left."}
...
![2D map](images/fig-p-som2-1.png){#fig-p-som1 fig-alt="Scatterplot on a 5x5 black grid, where points are spread a little at each node. The x axis is labelled 'map 1' and has no numbers, and the y axis is labelled 'map 2' and has no numbers. Points are coloured blue, yellow and red matching penguin species Adeli, Chinstrap and Gentoo. The top left is mostly blue, top right mostly yellow and bottom is mostly red." width=200 fig-align="center"}

![Map in 4D](images/p_som_47.png){#fig-p-som1 fig-alt="A single 2D projection from 4D of the penguins data, with points coloured according to species. Overlaid is a wireframe net, which stretches within and across the clusters. A circle and segment display shows the coefficients for each projection at bottom left."}
