## Non-linear dimension reduction {#sec-nldr}
...
Most techniques only require an interpoint similarity or distance matrix as the main ingredient, rather than the full $p$-dimensional data. A classic example where only this information is used is the morse code data reported by @morsecode, and available in the `xgobi` software [@xgobi]. The data, collected in an experimental study, contains the counts of the number of times that a letter is mistaken for another letter. However, here we focus on problems where the full $p$-dimensional data is available, so we can also compare structure perceived using the tour on the high-dimensional space, relative to structure revealed in the low-dimensional embedding.
...
![tour](gifs/swiss.gif){width=280 fig-alt="Rotating 3D plot where points are spread uniformly on a swiss roll shape."}
...
![Grand tour of the non-linear clusters data set, shows four clusters. Two are very small and spherical in shape. One is large, and has a sine wave shape, and the other is fairly small with a bent rod shape.](gifs/clusters_nonlin.gif){#fig-clusters-nonlin-html fig-alt="Animating plot of four variables, where different scatterplots of the projections show two small spherical clusters, one C-shape cluster and S-shaped stripe of points."}
:::
...
Two frames from a grand tour of the non-linear clusters data set, shows four clusters. Two are very small and spherical in shape. One is large, and has a sine wave shape, and the other is fairly small with a bent rod shape. {{< fa play-circle >}}
:::
:::
...
![Unibrow matches sine wave.](images/liminal-clusters-nonlin2.png){#fig-unibrow fig-alt="Screenshot of an interface labeled 'embed + tour,' showing two side-by-side panels with clusters of black dots forming abstract shapes on a white background. The left panel has a shaded vertical area and features a diagonal cluster of black dots. The right panel displays a denser, curved pattern of black dots resembling a distorted 'S' shape. The interface includes buttons at the bottom for 'Play,' 'Pause,' 'Restart,' and 'Controls,' with a 'Done' button in the top right and a 'Cancel' button in the top left."}
...
1. Change the seed using `set.seed` and re-run the code used for @fig-nldr-clusters. The resulting layout changes, right? What parts are persistent between different layouts, (e.g. always four clusters, always two small circular clusters, elongated cluster curves in different ways between layouts, ...)?
...
2. Examine the effect of various choices when making a 2D layout of the swiss roll data, using isomap and LLE:
    ...
    c. Use t-SNE and UMAP to layout the swiss roll data. How does this compare with isomap and LLE layouts?

3. This question uses the `penguins_sub` data.
...
4. Compute your best t-SNE and UMAP representations of the `aflw` data. Compare and contrast what is learned relative to the first two principal components, and a tour. Use `liminal` or  `detourr` to link with a tour to help you answer. 

5. The `cardinalR` package can be used to generate various high-dimensional shapes. Generate data with 5 irregularly shaped clusters, centering them at different points in five dimensions. Check what this looks like using a grand tour. If you were to sketch a 2D layout that describes your data, what would it look like? Use PCA, t-SNE and UMAP to make 2D layouts. Can you find a choice of parameters that comes close to your idealised sketch?

6. This question uses the `multicluster` data. 
    ...
    c. Use `liminal` or  `detourr` to link with a tour to explain how the layout does or doesn't capture the clusters. 

7. Use UMAP to create a 2D layout of the `sketches_train` data, `V1` to `V784`. Plot this with points coloured by `word` (what the person was asked to sketch). Use `liminal` or  `detourr` to link with a tour to help you answer. How well does the layout show clusters corresponding to the different classes?
