...
4. Repeat 1-3 until the data is grouped, and when no other separated cluster is visible in any projection. You may need to re-paint some points if they appear to be grouped incorrectly in a different projection, or paint more points that after spinning most likely belong to an existing group.
...
4. Use the spin-and-brush technique to identify the branches of the `fake_trees` data. The result should look something like @fig-fake-trees-sb. You can use the download button to save the data with the colours. Tabulate the `branches` id variable in the original data with the `colour` groups created from brushing, to see how closely you have recovered the original classes.
