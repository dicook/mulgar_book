...
uses the maximum interpoint distance, and average linkage uses the average of the interpoint distances. A good discussion on cluster analysis can be found in @HOML, on [Wikipedia](https://en.wikipedia.org/wiki/Cluster_analysis) or any multivariate textbook.
...
#| message: false
#| warning: false
#| fig-cap: "Hierarchical clustering on simulated data: (left) data, (middle) dendrogram, (right) dendrogram on the data. Nodes of the dendrogram indicated by `+` when it is drawn on the data."
#| fig-width: 9
#| fig-height: 3
library(ggplot2)
library(mulgar)
...
cl_hw <- hclust(dist(simple_clusters[,1:2]),
                method="ward.D2")
library(ggdendro)
ph <- ggdendrogram(cl_hw, labels = FALSE)
...
library(patchwork)
pd + ph + pdh + plot_layout(ncol=3)
```

@fig-hc-sim illustrates the hierarchical clustering approach, and the dendrogram representation of the results. 
