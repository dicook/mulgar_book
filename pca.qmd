...
::: insight
The 1D structure of the `pisa` data indicates that a student that scores well in math, tends to score well in reading and science, too. More interestingly, the regular shape of the data strongly indicates that it is "synthetic", simulated rather than observed.
:::
...
The function `pca_model()` from the `mulgar` package can be used to represent the model as a $k$-D wireframe plane. @fig-plane-box-model shows the models for the `plane` and `box` data, 2D and 3D respectively.

::: info
The pirpose of looking at the model in the data space is to check how well the model fits the data.
:::
...
```
::: {.content-hidden when-format="pdf"}
![PCA model of the pisa data. The 1D model captures the primary variation in the data and there is a small amount of spread in all directions away from the model. ](gifs/pisa_model.gif){#fig-pisa-model fig-alt="Something here" fig.align="center"}
:::
...
# with three dimensions we captureboth outliers in our graph
```