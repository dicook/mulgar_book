...
tree_branch <- keras::to_categorical(fake_trees$branches, num_classes = 10)
tree_input <- scale(fake_trees[,1:100])

ft_nn <- keras_model_sequential() 
ft_nn %>% 
...
ft_nn %>% compile(
  ...
  optimizer = optimizer_adam(learning_rate = 5*10E-7), 
  metrics = c('accuracy')
)
history_ft <- ft_nn %>% fit(
  tree_input, tree_branch, 
