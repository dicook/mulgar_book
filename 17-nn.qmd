...
# the first two components are clearly most important
activations_pcs <- as.data.frame(activations_pca$x)
...
# the first two components are clearly most important
images_pcs <- as.data.frame(images_pca$x)

ggplot(activations_pcs,
...
# instead what if we tour on the first 5 PCs
# using sage display works nicely for this
animate_sage(activations_pcs[,1:5], col = test_tags,
        pch = ".")

# might be nice to have double view of this and
# the output layer showing the probabilities, and have
# linked brushing between the two?
# might be a good idea to just look at a sample from
# the test data instead, maybe pick difficult points to
...
# can I make the lines thinner with the current implementation?
```

XXX why not shap? heatmap that people make for image recognition - is this easy in keras? show example of class wise and missclassified images

Sketches as example for overfitting? What would we look for? Make challenge excercise: can you do better than the random forest for this data?
