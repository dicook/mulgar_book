...
```{r eval=FALSE}
# this is not using the selected palette it seems?
animate_sage(images_pc[,1:5], col = test_tags,
        pch = ".", palette = "Set3")

animate_sage(activations_pc[,1:5], col = test_tags,
        pch = ".", palette = "Set3")

```

As with the first two principal components we get a much more spread out distribution in the original space. Nevertheless we can see differences between the classes, and that some groups are varying along specific directions in that space. Overall the activations space shows tighter clusters as expected after including the ReLU activation function, but the picture is not as neat as the first two principal components would suggest. While certain groups appear very compact even in this larger subspace, others vary quite a bit within part of the space. For example, images of XXX appear to be spanning a plane in this space.

Finally we will investigate the model performance through the missclassifications and uncertainty between classes. We start with the error matrix for the test observations.
...
fashion_test_pred_cat <- levels(test_tags)[
  apply(fashion_test_pred, 1,
        which.max)]
fashion_test_pred_cat <- factor(
  fashion_test_pred_cat,
  levels=levels(test_tags))
table(test_tags, fashion_test_pred_cat)
```
