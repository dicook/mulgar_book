...
\index{software!keras}
\index{software!tensorflow}

## Setting up the model {#sec-nn-model}
...
![Grand tour](gifs/p_split.gif){#fig-split-grand fig-alt="Animation showing a grand tour of the penguins data, with colour indicating species (red=Gentoo, blue=Adelie, yellow=Chinstrap) and shape indicating training (open circles) vs test (closed circles). The distribution of points in each group is reasonably similar regardless of projection." width="300"}

![Guided tour](gifs/p_split_guided.gif){#fig-split-guided fig-alt="Animation showing a guided tour of the penguins data, with colour indicating species (red=Gentoo, blue=Adelie, yellow=Chinstrap) and shape indicating training (open circles) vs test (closed circles). The distribution of points in each group is reasonably similar regardless of projection." width="300"}
...
![Grand tour](images/p_split.png){fig-alt="A scatter plot of a projection showing clusters of three penguin species—Adélie (blue), Chinstrap (yellow), and Gentoo (red)—based on multiple variables. Data points are marked as either training data (open circles) or test data (filled circles). The clusters are fairly distinct, with Adélie and Chinstrap appearing closer together, while Gentoo forms a distinct group. In the bottom-left corner, there is a small inset plot displaying variable contributions, with axes labeled 'bl' pointing to 11 o'clock, 'fl' pointing to 9 o'clock, 'bd' pointing to 3 o'clock and 'bm' pointing to 5 o'clock. A legend in the top-right corner indicates species colors, while another legend at the bottom-right differentiates between training (open circles) and test (filled circles) data." width="220"}

![Guided tour](images/p_split_guided.png){fig-alt="A scatter plot of a projection showing clusters of three penguin species—Adélie (blue), Chinstrap (yellow), and Gentoo (red)—based on multiple variables. Data points are marked as either training data (open circles) or test data (filled circles). The clusters are well-separated, with Adélie and Chinstrap appearing closer together, while Gentoo forms a distinct group. In the bottom-left corner, there is a small inset plot displaying variable contributions, with axes labeled 'bl' pointing to 2 o'clock, 'fl' pointing to 4 o'clock, 'bd' pointing to 10 o'clock and 'bm' pointing to 6 o'clock. A legend in the top-right corner indicates species colors, while another legend at the bottom-right differentiates between training (open circles) and test (filled circles) data." width="220"}

Evaluating the training/test split, where we expect that the two samples should roughly match. There are a few observations in the test set that are on the outer edges of the clusters, which will likely result in the model making an error in these regions. However, the two samples roughly match. {{< fa play-circle >}}
:::
:::
...
## Examining predictive probabilities {#sec-nn-pred-prob}
...
::: {.content-visible when-format="pdf"}
::: {#fig-fashion-gt-pdf layout-ncol="2"}

![Input space](images/fashion_images_gt_36.png){fig-alt="A scatter plot visualising high-dimensional fashion image data projected into two dimensions. Each point represents an individual image, colored by clothing category: Ankle boot (pink), Pullover (red), Trouser (yellow), Shirt (green), Coat (teal), Sandal (light blue), Sneaker (purple), Dress (peach), Bag (orange), and T-shirt/top (magenta). Clusters are visible although muddy, with some categories (like Trouser, Coat, and Sandal) forming elongated or compact groups, while others (such as Shirt and T-shirt/top) are more dispersed and overlapping. In the bottom-left corner, is the axis display showing the contribution of the first five principal components: PC1 and PC3 point to 1 o'clock, PC2 points to 9 o'clock, PC4 points to 6 o'clock, and PC5 points to 5 o'clock. A legend on the top-right maps each color to its corresponding fashion category." width="200"}

![Activations](images/fashion_activation_gt_126.png){fig-alt="A scatter plot visualising high-dimensional fashion activation data projected into two dimensions. Each point represents an individual image, colored by clothing category: Ankle boot (pink), Pullover (red), Trouser (yellow), Shirt (green), Coat (teal), Sandal (light blue), Sneaker (purple), Dress (peach), Bag (orange), and T-shirt/top (magenta). Clusters are visible, with some categories (like Trouser, Coat, and Sandal) forming distinct, elongated or compact groups, while others (such as Shirt and T-shirt/top) are more dispersed and overlapping. In the bottom-left corner, is the axis display showing the contribution of the first five principal components: PC1 points to 7 o'clock, PC2 points to 11 o'clock and PC4 points to 3 o'clock. A legend on the top-right maps each color to its corresponding fashion category." width="200"}

Comparison of the test observations in the first five principal components of the input space (left) and in the hidden layer activations (right). The activation function results in more clearly defined grouping of the different classes. {{< fa play-circle >}}
:::
:::
...
![](images/fashion_confusion_gt_227.png){fig-alt="A scatter plot showing a 2D projection of data in a 10D simplex with edges in light grey. Each point represents a fashion item, colored by category, and shows a tetrahedron shape containing vertices 0, 2, 3, 4, 6. Labeled vertices numbered 0 through 9 correspond to fashion item categories: T-shirt/top (blue), Pullover (green), Shirt (yellow), Coat (light green), and Other (purple). Dense connections exist between closely related categories, such as Pullover, Shirt, and Coat, suggesting misclassification potential. The 'Other' category (purple) is more dispersed across the plot, linking pairs of nodes. A legend in the top-right corner identifies the color-coded categories." width="130"}

![](images/fashion_confusion_gt_258.png){fig-alt="A scatter plot showing a 2D projection of data in a 10D simplex with edges in light grey. Each point represents a fashion item, colored by category. Labeled vertices numbered 0 through 9 correspond to fashion item categories: T-shirt/top (blue), Pullover (green), Shirt (yellow), Coat (light green), and Other (purple). Dense connections exist between closely related categories, such as Pullover, Shirt, and Coat, suggesting misclassification potential. The 'Other' category (purple) is more dispersed across the plot, linking pairs of nodes. A legend in the top-right corner identifies the color-coded categories." width="130"}

![](images/fashion_confusion_gt_275.png){fig-alt="A scatter plot showing a 2D projection of data in a 10D simplex with edges in light grey. Each point represents a fashion item, colored by category. Labeled vertices numbered 0 through 9 correspond to fashion item categories: T-shirt/top (blue), Pullover (green), Shirt (yellow), Coat (light green), and Other (purple). Dense connections exist between closely related categories, such as Pullover, Shirt, and Coat, suggesting misclassification potential. The 'Other' category (purple) is more dispersed across the plot, linking pairs of nodes. A legend in the top-right corner identifies the color-coded categories." width="130"}

![](images/fashion_confusion_gt_342.png){fig-alt="A scatter plot showing a 2D projection of data in a 10D simplex with edges in light grey. Each point represents a fashion item, colored by category. Labeled vertices numbered 0 through 9 correspond to fashion item categories: T-shirt/top (blue), Pullover (green), Shirt (yellow), Coat (light green), and Other (purple). Dense connections exist between closely related categories, such as Pullover, Shirt, and Coat, suggesting misclassification potential. The 'Other' category (purple) is more dispersed across the plot, linking pairs of nodes. A legend in the top-right corner identifies the color-coded categories." width="130"}

![](images/fashion_confusion_gt_405.png){fig-alt="A scatter plot showing a 2D projection of data in a 10D simplex with edges in light grey. Each point represents a fashion item, colored by category. Labeled vertices numbered 0 through 9 correspond to fashion item categories: T-shirt/top (blue), Pullover (green), Shirt (yellow), Coat (light green), and Other (purple). Dense connections exist between closely related categories, such as Pullover, Shirt, and Coat, suggesting misclassification potential. The 'Other' category (purple) is more dispersed across the plot, linking pairs of nodes. A legend in the top-right corner identifies the color-coded categories." width="130"}

![](images/fashion_confusion_gt_433.png){fig-alt="A scatter plot showing a 2D projection of data in a 10D simplex with edges in light grey. Each point represents a fashion item, colored by category, and shows a tetrahedron shape containing vertices 0, 2, 3, 4, 6. Labeled vertices numbered 0 through 9 correspond to fashion item categories: T-shirt/top (blue), Pullover (green), Shirt (yellow), Coat (light green), and Other (purple). Dense connections exist between closely related categories, such as Pullover, Shirt, and Coat, suggesting misclassification potential. The 'Other' category (purple) is more dispersed across the plot, linking pairs of nodes. A legend in the top-right corner identifies the color-coded categories." width="130"}

Selected projections from a tour of the votes matrix for the fashion MNIST test observations, focusing on a subset of items. Often observations get confused between two of the classes, this appears as points falling along one of the edges, for example some Shirts look more like T-shirts/tops, while others get confused with Coats. We can also notice that a subset of three other classes not mapped to colors as very separate from this group. {{< fa play-circle >}}
:::
:::
...
1.  The problem with the NN model fitted to the penguins is that the Gentoo are poorly classified, when they should be perfectly predictable due to the big gap between class clusters. Re-fit the NN, with two nodes in the hidden layer) to the penguins data, to find a better model that appropriately perfectly predicts Gentoo penguins. (You might find the easiest way to get a better model is changing the random seed and re-fitting, as illustrated in @wickham2015, Figures 21-24.) Support this by plotting the model (using the hidden layer), and the predictive probabilities as a ternary plot. Do the SHAP values also support that `bd` plays a stronger role in your best model? (`bd` is the main variable for distinguishing Gentoo's from the other species, particularly when used with `fl` or `bl`.)
