...
model_fashion_mnist %>% evaluate(test_images, test_labels)

fashion_nn_wgts <- get_weights(model_fashion_mnist, trainable=TRUE)
...
# to look at this new feature space we first reduce
...
train_tags <- class_names[train_labels + 1]

ggplot(activations_pcs,
       aes(PC1, PC2, color = train_tags)) +
  geom_point(shape = ".")

# instead what if we tour on the first 5 PCs
animate_xy(activations_pcs[,1:5], col = train_tags,
        pch = ".")

# too slow when animating like this, maybe better with
# detourr or other implementation?
