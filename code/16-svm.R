...
dc1 <- rmvn(n=n1, p=2, mn=c(-2, -2), vc=vc1)
vc2 <- matrix(c(1, -0.4, -0.4, 1)*2, ncol=2, byrow=TRUE)
n2 <- 138
dc2 <- rmvn(n=n2, p=2, mn=c(2, 2), vc=vc2)
df1 <- data.frame(x1=mulgar:::scale2(c(dc1[,1], dc2[,1])), 
                 x2=mulgar:::scale2(c(dc1[,2], dc2[,2])), 
                 cl = factor(c(rep("A", n1), 
                               rep("B", n2))))
dc1 <- sphere.hollow(p=2, n=n1)$points*3 + 
  c(rnorm(n1, sd=0.3), rnorm(n1, sd=0.3))
dc2 <- sphere.solid.random(p=2, n=n2)$points
df2 <- data.frame(x1=mulgar:::scale2(c(dc1[,1], dc2[,1])), 
                  x2=mulgar:::scale2(c(dc1[,2], dc2[,2])), 
                  cl = factor(c(rep("A", n1), 
                               rep("B", n2))))
