...
Based on the dimension of the projection basis there are natural ways to display the projected data. If $d=2$ it is most common that we would use a scatterplot, as shown in (as done in `r ifelse(knitr::is_html_output(), '@fig-tour-types-html', '@fig-tour-types-pdf')`. 
