...
With data in two dimensions, we can see different ways that observations group into clusters in a scatterplot display. For higher dimensional data we can use the tour to perceive clustering, this is demonstrated for clusters in 2D and projections onto 1D in `r ifelse(knitr::is_html_output(), '@fig-explain-1D-html', '@fig-explain-1D-pdf')`. We may find projections where groups are separated in a single view, or we can also track the movement patterns during a 2D tour animation to identify differences between groups, see @fig-trails.

For a first understanding it can be useful to observe these aspects in an example where clearly separated groups exist and can be labeled, for example the three penguin species in `palmerpenguins` [@R-palmerpenguins]. @fig-penguins-tour shows how in this case a 2D tour of the 4D data can reveal different aspects in the data, we can see that one species (Gentoo) is clearly separated from the other two species, and differences between Adelie and Chinstrap penguins become clearer by following the movement patterns.

We can also use the tour to identify more complicated cluster structures, for example `r ifelse(knitr::is_html_output(), '@fig-clusters-nonlin-html', '@fig-clusters-nonlin-pdf')` shows how clusters of different shapes and sizes can be distinguished using a grand tour.

With larger data and more subtle cluster structure, a guided tour might be useful to identify projections that show separation between groups, for example the holes index guided tour can often reveal grouping in the data. The final view identified with a guided tour can then be investigated further using a local or radial tour, to better understand the clustering.

Once a clustering solution has been found through some algorithm, we can also use the tour to investigate the solution, and different approaches and examples are shown in this chapter.

## Exercises {-}

1. Use the following data to answer these questions:
...
a. Compute the Euclidean distance between cases `a1`, `a2`, `a3`, `a4`.

b. Compute the correlation distance (as defined above) between cases `a1`, `a2`, `a3`, `a4`.


c. Which two points have the (a) biggest (b) smallest Mahalanobis (statistical) distance, assuming that the covariance matrix is:
...
d. Is the ordering of distance between cases the same if Manhattan distance is used instead of Euclidean?

e. Compute the Chebychev distance between cases `a1`, `a2`, `a3`, `a4`.

f. Compute Bray-Curtis distance between cases `a1`, `a2`, `a3`, `a4`.

g. Make a plot of the data, and write a paragraph describing how the different distance metrics agree and disagree on how close or far the cases are from each other. 
...
2. From these plots of 2D data sketch what you would consider the clusters to be? (In the next chapters keep these data in mind, and think about whether the clusters that would result from each method would match your sketches.)
...
d1 <- gau_clust(n=402, 
                num_clust=6, 
                num_dims=2, 
                mean=matrix(runif(6*2), ncol=2), 
                var_vec=rep(0.03, 6), 
                num_noise=0) 
d1 <- as.data.frame(d1)
cp1 <- ggplot(d1, aes(x=V1, y=V2)) + 
  geom_point() +
  theme_minimal() +
  theme(aspect.ratio=1,
        axis.text = element_blank(),
        ...
        panel.border = element_rect("black", fill=NA, 
             linewidth = 0.5))
d2 <- curvy_cycle(n=402, 
                  num_noise=0) 
d2 <- as.data.frame(d2) |>
  mutate(V4 = V1/sqrt(2) + 3*V2/sqrt(2),
         V5 = -V3/sqrt(2) + 3*V1/sqrt(2))
cp2 <- ggplot(d2, aes(x=V4, y=V5)) + 
  geom_point() +
  theme_minimal() +
  theme(aspect.ratio=1,
        axis.text = element_blank(),
...
cp3 <- ggplot(d3, aes(x=V1, y=V2)) + 
  ...
  theme(aspect.ratio=1,
        ...
        panel.border = element_rect("black", fill=NA, 
             linewidth = 0.5))
d4 <- mobius_clust(n=405, 
           num_noise=0)
d4 <- as.data.frame(d4)
cp4 <- ggplot(d4, aes(x=V1, y=V2)) + 
  geom_point() +
  theme_minimal() +
  theme(aspect.ratio=1,
        axis.text = element_blank(),
...
::: {.hidden}
Probably we would expect

- 6 similar size circular clusters
- 3 wonky rings
- 9 linear pieces
- 2, one a small solid circle, the other a ring with varying thickness 
:::
:::

3. Use a tour to look at each of the challenge data sets c1, c2, . . . , c7 from the mulgar package. For which ones do you observe clearly separated clusters? Do you notice any shapes that should be taken into account for a cluster analysis?

4. Generate data using the `cardinalR` package that contains:

a. Four Gaussian clusters
b. Circular data with three clusters
c. Your choice

For which ones do you observe clearly separated clusters? Do you notice any shapes that should be taken into account for a cluster analysis?
