# Introduction to clustering {#sec-intro-clust}
...
1. Use the data and variance-covariance matrix given below to answer these questions:
    ...
    c. Which two points have the (a) biggest (b) smallest Mahalanobis (statistical) distance, assuming that the covariance matrix is $\Sigma$. 
...
2. From the plots of 2D data in @fig-what-clusters sketch what you would consider the clusters to be? (In the next chapters keep these data in mind, and think about whether the clusters that would result from each method would match your sketches.)
...
cp4 <- ggplot(d4, aes(x=x1, y=x2)) + 
  ...
  theme(aspect.ratio=1,
        ...
        panel.border = element_rect("black", fill=NA, 
             linewidth = 0.5))
cp1 + cp2 + cp3 + cp4 + plot_layout(ncol=4)
```
...
4. Generate data using the `cardinalR` package that contains:
    a. Four Gaussian well-separated clusters
    b. Data containing with three clusters that are circle shapes
    c. Your choice 
