...
- Penguin data: we can see the three clusters, and in particular if we have the species mapped to color we can follow how the clusters have different movement patterns more easily
- Guided tour can be helpful, often the holes index will identify views that show clustering
- Combining with the radial tour we can check what variables are important for the grouping
- German election data from Leisch reference: Small cluster clearly separate from most points, but it can also be useful to break up the larger group into a few smaller ones (what are the shapes we see?)
- Example with unusual shapes, where we need to be careful with distance measure: clusters_nonlin
