...
::: insight
We use the radial tour to examine if a variable is important when separating the two classes: when we can see the plane turning over as we change how the variable is contributing to the projection. It means that depending on the observed value in this variable, observations can move from one to the other side of the separating hyperplane.
:::
...
1. Generate a small subset from the `bushfire` data: we keep the variables `log_dist_cfa`, `log_dist_road` and `cause`, and we select only observations where `cause` is either lightning or arson. Fit both an LDA and a linear SVM model to separate the two classes and show the two decision boundaries together with the data. What can you say about the model assumptions for LDA for this data? What differences in decision boundaries are there? Can they be understood in terms of the model definitions?
...
4. XX nonlinear when separating two types of sketches (banana vs cactus?) - PCA + radial SVM + slice tour of the predicted classes to see shape of decision boundary
5. Repeat but with third class?
