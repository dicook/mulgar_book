...
dup <- duplicated(psub)
psub <- penguins_cl %>%
  select(bl, bd, species, cl_w) %>%
  filter(!dup) %>%
  mutate(cl_w = as.numeric(as.character(cl_w)))
phull <- cxhull(as.matrix(psub[,1:2]))$edges
phull <- as.data.frame(phull)
colnames(phull) <- c("from", "to") 
...
phull <- as.data.frame(phull)
colnames(phull) <- c("from", "to") 
phull$cl_w <- factor(phull$cl_w, levels=3:1)
phull_segs <- data.frame(x = psub$bl[phull$from],
                         y = psub$bd[phull$from],
                         xend = psub$bl[phull$to],
                         yend = psub$bd[phull$to],
...
animate_xy(phull[,1:4], col=phull$species,
           edges=phull$edges)
```
