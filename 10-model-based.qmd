...
To draw ellipses in any dimension, the procedure is to sample points uniformly on a sphere, and then transform this into a sphere using the inverse of the variance-covariance matrix. The `mulgar` function `mc_ellipse()` does this for each cluster in the fitted model.
\index{data!penguins}
...
2. Fit model-based clustering to the `clusters` data. Does it suggest the data has three clusters? Using the tour examine the best model. How well does this fit the data?
3. Fit model-based clustering to the `multicluster` data. Does it suggest the data has six clusters? Using the tour examine the best model. How well does this fit the data?
4. Fit model-based clustering to the `fake_trees` data. Does it suggest that the data has 10 clusters? If not, why do you think this is? Using the tour examine the best model. How well does this fit the branching structure?
...
6. Use model-based clustering on the challenge data sets, `c1` from the `mulgar` package. What model and how many clusters would you expect to be the best fit? Explain why the best model fits the cluster structure or not.
