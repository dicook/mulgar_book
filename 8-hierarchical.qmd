...
::: {.content-visible when-format="pdf"}
::: {#fig-penguins-gt-pdf layout="[[33, 33, 33]]"}

![](images/penguins_gt_59.png){fig-alt="A scatterplot of a 2D projection. You can see three circular clusters, one further apart from the other two."}

![](images/penguins_gt_32.png){fig-alt="A scatterplot of a 2D projection. You can see one large blob, and an outlier."}

![](images/penguins_gt_105.png){fig-alt="A scatterplot of a 2D projection. You can see two clusters, one smaller and one twice the size. Both are lying along an x=-y direction."}

Three frames from a grand tour being used to check for clusters, the shape of clusters and for nuisance observations and variables. Here the penguins data looks like it has possibly three elliptical clusters, one more separated than the other two, with some nuisance observations (outliers). {{< fa play-circle >}}
:::
:::
...
::: {.content-visible when-format="pdf"}
::: {#fig-penguins-hfly-pdf layout-ncol=2 fig-pos="H"}
...
Dendrograms for Wards and single linkage of the penguins data, shown in 2D (top) and in 4D (middle), and the three-cluster solution of each. {{< fa play-circle >}}
:::
:::
